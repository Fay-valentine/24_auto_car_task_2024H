...
static uint8_t circle_count=1;//当前圈数
static float initial_yaw=0.0f;//Mode4_Init时采样的初始航向角，每圈开始时重置target_yaw为此值

/**
 * @brief 每完成一圈后的状态重置
 */
void Mode4_state_reset(void)
{
    point_count=POINT_A;
    YawPID_SetTarget(&yaw_pid, initial_yaw);//重置目标航向为初始航向，避免上一圈累积的target_yaw偏差影响下一圈
	Mode4_flagReset();//重置标志位
}
...
void Mode4_Init(void)
{
	...
	Stop_Num=17;				//兜底停车点（实际不可达，主要靠circle_count停车）
	circle_count=1;				//重置圈数，防止上次Mode4运行残留
	sampleYaw(&yaw_pid);		//采样一次目标航向角
	initial_yaw=YawPID_GetTarget(&yaw_pid);//保存初始航向角，供每圈开始时重置
	//显示相应信息
...
void Mode4_Loop(void)
{
	if(getModeLoopFlag())//只有在初始化设置了 循环开启flag 为1时 才执行循环Loop
	{
		...
		else if (point_count == POINT_B)
		{
			...
			//1.逆时针旋转40°左右
		if(B_turn_first==0 && turnByAngle(&yaw_pid, -1, turn_angle+yaw_adjust)==1)
		{
			B_turn_first=1;
			Motion_Stop(STOP_BRAKE);//旋转完后刹车
			/* 余弦补偿：根据转弯后的航向误差放大里程目标 */
			...
			{
				if(B_turn_second==0 && turnByAngle(&yaw_pid, 1, turn_angle)==1)
				{
					Motion_Stop(STOP_BRAKE);//旋转完后刹车
        ...
        else if(point_count==POINT_FINAL)
		{
			circle_count++;//增加圈数
		    if(circle_count==5)//完成4圈后停车
		    {
		        Mode4_Exit();
		        return;//已退出模式，不再执行后续状态重置
		    }
		    //刷新状态
		    Mode4_state_reset();
		}
